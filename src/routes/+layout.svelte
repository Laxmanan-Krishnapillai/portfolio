...
<svelte:window on:mousemove={(e)=>{
    ...
    console.log(e)
    if(e.target.tagName === "A" && out) {
        gsap.to("#cursor", {ease: "power4.out", duration: 0.3,backgroundColor: "transparent"})
        out = false;
    } else {
        gsap.to("#cursor", {ease: "power4.out", duration: 0.3,backgroundColor: "rgb(15 23 42)"})
        out = true;
    }
}}/>
<div id="cursor" class="pointer-events-none w-10 h-10 rounded-full bg-slate-900 border-2 border-slate-900" />
